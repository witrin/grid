 ...
 */
class ToggleItemVisibilityActionProvider implements FormDataProviderInterface
{
    /**
     * Add data
     *
        ...
        foreach ($result['customData']['tx_grid']['items']['children'] as &$item) {
            if (
                ...
                if (!empty($item['processedTca']['ctrl']['enablecolumns']['disabled'])) {
                    ...
                    if (
                        $enableField && $item['processedTca']['columns'][$enableField] &&
                        !$item['processedTca']['columns'][$enableField]['exclude'] ||
                        $this->getBackendUserAuthentication()->check(
                            'non_exclude_fields',
                            $item['tableName'] . ':' . $enableField
                        )
                    ) {
                        $toggle = (bool)$item['databaseRow'][$enableField];
                        $item['customData']['tx_grid']['actions'][$toggle ? 'unhide' : 'hide'] = BackendUtility::getLinkToDataHandlerAction(
                            '&data[' . $item['tableName'] . '][' . $item['vanillaUid'] . '][' . $enableField . ']=' . (int)!$toggle
                        );
                    }
                }
            }
