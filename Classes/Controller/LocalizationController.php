...
class LocalizationController
{
    ...
    public function getLanguages(ServerRequestInterface $request, ResponseInterface $response)
    {
        ...
        $languageUid = (int)$params['languageUid'];
        ...
        $elementsInColumnCount = $this->localizationRepository->getLocalizedRecordCount($containerUid, $areaUid, $languageUid, $containerTable, $containerField);

        if ($elementsInColumnCount === 0) {
            $fetchedAvailableLanguages = $this->localizationRepository->fetchAvailableLanguages($containerUid, $areaUid, $languageUid, $containerTable, $containerField);
            ...
            $result = $this->localizationRepository->fetchOriginLanguage($containerUid, $areaUid, $languageUid, $containerTable, $containerField);
