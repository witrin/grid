...
class DragDrop {
  ...
  protected isDropAllowed(zone: Element, item: Element) {
    const target = Math.abs(Number($(zone).attr('data-target')));
  ...
  protected onDrop(item: Element, zone: Element, event: any): void {
    ...
    const target = parseInt($(zone).attr('data-target'), 10);
    const parameters = JSON.parse($(zone).attr('data-parameters'));
    ...
    if (this.getDragType(item) === DragType.Record) {
      ...
      const values = JSON.parse($(item).attr('data-values'));
